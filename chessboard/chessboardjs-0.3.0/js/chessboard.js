...
function validSquare(square) {
  ...
  return (square.search(/^[a-j][1-8]$/) !== -1);
}
  ...
  for (var i = 0; i < BOARD_SIZE; i++) {
    ...
    for (var j = 0; j < row.length; j++) {
      ...
      if (row[j].search(/[1-8]/) !== -1) {
        var emptySquares = parseInt(row[j], 8);
...
window['ChessBoard'] = window['ChessBoard'] || function(containerElOrId, size, cfg) {
'use strict';
var BOARD_SIZE = size;
cfg = cfg || {};
...
function buildBoard(orientation) {
  ...
  for (var i = 0; i < BOARD_SIZE; i++) {
    ...
    for (var j = 0; j < BOARD_SIZE; j++) {
      ...
      squareColor = (j % 2 == 0 ? (i % 2 == 0 ? 'white' : 'black') : (i % 2 == 0 ? 'black' : 'white'));
    }
    html += '<div class="' + CSS.clearfix + '"></div></div>';

    squareColor = (i % 2 == 0 ? 'white' : 'black');
